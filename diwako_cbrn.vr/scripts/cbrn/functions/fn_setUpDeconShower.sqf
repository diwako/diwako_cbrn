...
if (local _shower) then {
    _shower setVariable ["cbrn_water", cbrn_deconWaterTime, true];
};
...
private _action = ["cbrn_turn_on", "Turn on","",{
    ...
    private _remainingWater = _target getVariable ["cbrn_water", cbrn_deconWaterTime];
    ...
    !(_target getVariable ["cbrn_on", false]) && {(_target getVariable ["cbrn_water", cbrn_deconWaterTime]) > 0}
...
_action = ["cbrn_turn_off", "Turn off","",{
    ...
    private _waterRemaining = ((_target getVariable ["cbrn_water", cbrn_deconWaterTime]) - _diff) max 0;
